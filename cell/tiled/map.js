...
export class InvalidLayerError extends Error {
	/**
	 * @param {number} layerID The requested layer ID
	 */
	constructor(layerID) {
		super(`The requested layer (${layerID}) is invalid`);
		/** @type {number} */
		this.layer = layerID;
	}
}
...
export class TiledMap {
	 ...
	 * @param {string} path
	 */
	constructor(options, customProperties = null, path = "") {
		this.path = path;
		/** @type {TiledTileset[]} */
		this.tilesets = [];
		/** @type {TiledLayer[]} */
	...
	tileAt(x, y, layer) {
		if(this.layers[layer] == undefined) {
			throw new InvalidLayerError(layer);
		}
	...
	/**
	 * Loads a map from the specified path and returns a new `TiledMap`
	 * from it
	...
	static fromFile(path) {
		...
		switch(FS.extensionOf(path).toLowerCase()) {
			...
			case ".json":
				return TiledMap.fromJSON(str, path);
			default:
	...
	static fromJSON(jsonIn, path = "") {
		...
		for(const tileset of tilesetsRaw) {
			map.tilesets.push(
				TiledTileset.fromJSON(tileset, FS.directoryOf(map.path))
			);
		}
		return map;
	}
