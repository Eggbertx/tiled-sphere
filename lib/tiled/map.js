...
export class TiledMap {
	...
	static fromJSON(jsonStr) {
		...
		let mapOptions = {};
		for(const key in mapOptionsKeys) {
			mapOptions[key] = jsonMap[key];
		}
		
		let customProperties = TiledProperties.fromJSONNode(jsonMap["properties"])

		if(jsonMap["properties"] instanceof Array) {
			for (const property of jsonMap["properties"]) {
				// SSj.log(property);
			}
		}
		let tilesets = [];
		...
		let map = new TiledMap(mapOptions)
		return map;
	}
}
